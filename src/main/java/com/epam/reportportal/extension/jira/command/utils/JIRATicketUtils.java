...
public class JIRATicketUtils {
  ...
  public static IssueInput toIssueInput(JiraRestClient client, Project jiraProject,
    ...
    GetCreateIssueMetadataOptions options =
        new GetCreateIssueMetadataOptionsBuilder().withExpandedIssueTypesFields()
            .withProjectKeys(jiraProject.getKey()).build();
    Iterator<CimProject> projects =
        client.getIssueClient().getCreateIssueMetadata(options).claim().iterator();
    ...
    CimIssueType cimIssueType =
        EntityHelper.findEntityById(project.getIssueTypes(), issueType.get().getId());
